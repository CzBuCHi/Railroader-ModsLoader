...
public class TestsMemoryFileSystem
{
    ...
    public void Constructor_CreateCorrectMocks() {
        ...
        // Assert
        sut.File.Should().BeOfType(Substitute.For<IFile>().GetType());
        sut.Directory.Should().BeOfType(Substitute.For<IDirectory>().GetType());
    ...
    public void Constructor_MockReturnsCorrectValues() {
        ...
        // Assert
        mock.File.Should().Be(sut.File);
        mock.Directory.Should().Be(sut.Directory);
